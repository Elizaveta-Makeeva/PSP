                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <div class="card">
                            <div class="card-body">
                                <h3>Контакты</h3>
                                <p><strong>Телефон:</strong> ${data['contacts']['phone'] || 'Не указан'}</p>
                                <p><strong>Сайт:</strong> <a href="${data['contacts']['website'] || '#'}" target="_blank">${data['contacts']['website'] || 'Не указан'}</a></p>
                                <p><strong>Часы работы:</strong> ${data['contacts']['hours'] || 'Не указаны'}</p>
                            </div>
                        </div>
                    </div>
                </div>
                    <button class="btn btn-secondary" id="back-btn">← Вернуться на главную</button>
                    <button class="btn btn-outline-danger me-3" id="like-btn">❤️ ${likesCount}</button>
            </div>
            `
        )
    }
